...
# get the proportions for each gene
# group by gene 
by_gene <- filterbymindist_allchr %>% group_by(gene)

# each gene gets a number of sig and number of nonsig
chr_cpg_TSS_50kb_sig_counts <- by_gene %>% summarise(sig_count=sum((qvalue < 0.05) == TRUE ), non_sig_count =sum((qvalue > 0.05) == TRUE ))

# we want to filter out any genes that have less than 10 tested sites because it will sway the proportions
less_than_10_index <- which(chr_cpg_TSS_50kb_sig_counts$sig_count + chr_cpg_TSS_50kb_sig_counts$non_sig_count < 10)

# remove those 
chr_cpg_TSS_50kb_sig_counts_greater10 <- chr_cpg_TSS_50kb_sig_counts[-less_than_10_index,]
# save total sites tested near gene
chr_cpg_TSS_50kb_sig_counts_greater10$total_sites <- chr_cpg_TSS_50kb_sig_counts_greater10$sig_count + chr_cpg_TSS_50kb_sig_counts_greater10$non_sig_count
# get the proportion significant
chr_cpg_TSS_50kb_sig_counts_greater10$proportion_sig <- chr_cpg_TSS_50kb_sig_counts_greater10$sig_count/chr_cpg_TSS_50kb_sig_counts_greater10$total_sites


# save the empirical proportions
write.table(chr_cpg_TSS_50kb_sig_counts_greater10, file = "allchr_cpg_TSS_50kb_sig_counts_empirical_proportions.txt")
